...
v2 - Adding Numba and np.array variants (for parallelization, jit)
...
def convertListERsToNpArrayERs(listERs1,listERs2):
    overallStartTime = time.time_ns()
    npArrayERs1 = []
    npArrayERs2 = []
    if len(listERs1) < len(listERs2):
        smallerSize = len(listERs1)
    else:
        smallerSize = len(listERs2)
    for i in range(0,smallerSize):
        npArrayERs1.append(np.array(listERs1[i]))
        npArrayERs2.append(np.array(listERs2[i]))
    overallFinishTime = time.time_ns()
    overallDuration = (overallFinishTime - overallStartTime)   # naonseconds
    return npArrayERs1,npArrayERs2,overallDuration
...
def compareSetNumbaERs(setERs1,setERs2):
    # this routine uses unsorted set, and 'if short set item in longSet'
    if len(setERs1) < len(setERs2):
        smallerSize = len(setERs1)
    else: 
        smallerSize = len(setERs2)
    counts = [0 for _ in range(0,smallerSize)]
    durations = []
    overallStartTime = time.time_ns()
    for i in range(0,smallerSize):
        startTime = time.time_ns()
...
def compareNpArrayNumbaERs(npArrayERs1,npArrayERs2):
    # this routine uses unsorted set, and 'if short set item in longSet'
    # print("len(npArrayERs1),type(npArrayERs1): ",len(npArrayERs1),type(npArrayERs1))
    # print("npArrayERs1: ",npArrayERs1)
    # print("len(npArrayERs1[0]: ",len(npArrayERs1[0]))
    if len(npArrayERs1) < len(npArrayERs2):
        smallerSize = len(npArrayERs1)
    else:
        smallerSize = len(npArrayERs2)
    counts = [0 for _ in range(0,smallerSize)]
    durations = []
    overallStartTime = time.time_ns()
    for i in range(0,smallerSize):
...
def compareSetNumbaJITERs__TEMP(setERs1,setERs2):
    # this routine uses unsorted set, and 'if short set item in longSet'
    if len(setERs1) < len(setERs2):
        smallerSize = len(setERs1)
    else: 
...
def compareNpArrayNumbaERs__TEMP(npArrayList1,npArrayList2):
    # this routine uses unsorted set, and 'if short set item in longSet'
    if len(npArrayList1) < len(npArrayList2):
        smallerSize = len(npArrayList1)
    else:
        smallerSize = len(npArrayList2)
    counts = [0 for _ in range(0,smallerSize)]
    durations = []
    overallStartTime = time.time_ns()
    for i in range(0,smallerSize):
...
def main():
    ...
    erSizesList = [256,512,1024] #,2048,4096,8192,16384]
    nodePoolSizesList = [2**20]  #2**10,2**12,2**14,2**16] #,2**17,2**19]
    ...
    timingResults_fileName = 'data/timingResults-erCompare-withNumba1M-'+timeId+'.trecPkl'
    ...
    for i in range(0,len(erSizesList)):
        for j in range(0,len(nodePoolSizesList)):
            ...
            # build np.array ERs
            npArrayERs1, npArrayERs2, overallDuration = convertListERsToNpArrayERs(baseERs1,baseERs2)
            erDict['npArrayERs'] = {'description':'npArrays',\
                                 'lengths':[erSize1,erSize2],'nodePool':nodePool,\
                                 'ers': [npArrayERs1,npArrayERs2]} 
            averageDuration = (overallDuration/quantity/1000000)/2     # divide by two because there are two sets lists being converted
            print("")
            print("Convert Lists to npArrays")
            print("  overallDuration/quantity(ms): ",overallDuration/1000000/2, quantity)
            print("  averageDuration(ms): ",averageDuration)
            dictItem['npArrayConversionResults'] = [averageDuration]                    
            ...
            # npArray compare with Numba
            counts,durations,overallDuration = compareNpArrayNumbaERs(npArrayERs1,npArrayERs2)
            meanDuration =mean(durations)/1000000
            averageDuration = overallDuration/quantity/1000000
            stdevDuration = stdev(durations)/1000000
            print("")
            print("npArray ER dot-product")
            # print("counts/durations(ns): ",counts,durations)
            print("  overallDuration(ms)/quantity: ",overallDuration/1000000, quantity)
            print("  meanDuration(ms): ",meanDuration)
            print("  stdevDuration(ms): ",stdevDuration)
            print("  averageDuration(ms): ",averageDuration)   
            dictItem['npArraytResults'] = [meanDuration,stdevDuration,averageDuration]            
            
            
            # set compare with Numba
            counts,durations,overallDuration = compareSetNumbaERs(setERs1,setERs2)
            meanDuration =mean(durations)/1000000
            averageDuration = overallDuration/quantity/1000000
            stdevDuration = stdev(durations)/1000000
            print("")
            print("setNumba ER dot-product")
            # print("counts/durations(ns): ",counts,durations)
            print("  overallDuration(ms)/quantity: ",overallDuration/1000000, quantity)
            print("  meanDuration(ms): ",meanDuration)
            print("  stdevDuration(ms): ",stdevDuration)
            print("  averageDuration(ms): ",averageDuration)   
            dictItem['setNumbatResults'] = [meanDuration,stdevDuration,averageDuration]    
